...
export async function selectModel(client: "claude" | "codex", choices: ModelProvider[]): Promise<ModelProvider> {
  ...
  const render = () => {
    ...
    choices.forEach((choice, index) => process.stdout.write(`${index === selected ? "❯" : " "} ${choice.provider}/${choice.model} (${choice.protocol})\n`));
  ...
  return new Promise((resolve, reject) => {
    const onKeypress = (_: string, key: { name?: string; ctrl?: boolean; }) => {
      ...
      if (key.name === "return" || key.name === "enter") { const model = choices[selected]; cleanup(); process.stdout.write("\x1b[2J\x1b[H"); resolve(model); return; }
      render();
    };
