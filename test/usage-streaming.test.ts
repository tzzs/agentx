...
/** A stand-in `ServerResponse` whose `writes()` returns everything the pipe emitted. */
function fakeResponse() {
  const chunks: string[] = [];
  const response = { writeHead() {}, write(value: string) { chunks.push(value); }, end() {}, on() {} };
  return { response, writes: () => chunks.join("") };
}

/** An upstream `Response` replaying `chunks` as its SSE body. */
...
test("pipeResponsesStream reports usage from the final chunk", async () => {
  ...
  await pipeResponsesStream(upstream, response as never, "gpt-5.6-luna", opts);
  ...
  assert.match(writes(), /"type":"text_delta","text":"OK"/);
});
...
test("pipeResponsesStream reads top-level chat completions usage for chat upstreams", async () => {
  ...
  await pipeResponsesStream(upstream, response as never, "deepseek-v4-pro", opts);
...
test("pipeChatStreamToResponses reports usage from the final chunk", async () => {
  ...
  const { response, writes } = fakeResponse();
  const usages: TokenUsage[] = [];
  await pipeChatStreamToResponses(upstream, response as never, "deepseek-v4-pro", { provider: "deepseek", model: "deepseek-v4-pro", protocol: "chat-completions", onUsage: (usage) => usages.push(usage) });
  assert.deepEqual(usages[0], { provider: "deepseek", model: "deepseek-v4-pro", inputTokens: 50, outputTokens: 10, totalTokens: 60 });
  assert.match(writes(), /"text":"OK"/);
});
...
test("pipeResponsesPassthrough forwards chunks and captures usage", async () => {
  ...
  const { response, writes } = fakeResponse();
  const usages: TokenUsage[] = [];
  await pipeResponsesPassthrough(upstream, response as never, "gpt-5.6-luna", { provider: "opencode", model: "gpt-5.6-luna", protocol: "responses", onUsage: (usage) => usages.push(usage) });
  assert.equal(writes().includes("output_text"), true);
...
test("stream pipes capture cache and reasoning tokens from final usage", async () => {
  ...
  const { response } = fakeResponse();
  const usages: TokenUsage[] = [];
  await pipeChatStreamToResponses(upstream, response as never, "gpt-4o", { provider: "openai", model: "gpt-4o", protocol: "chat-completions", onUsage: (usage) => usages.push(usage) });
