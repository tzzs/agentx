...
test("lists issues and reports a not-ready status", () => {
  const output = renderDoctor({ ...base, apiKeyFound: false, issues: ["Set OPENCODE_GO_API_KEY before starting the adapter, or pass --api-key <key>."] });
  assert.match(output, /Status: Not ready/);
  assert.match(output, /- Set OPENCODE_GO_API_KEY/);
});
